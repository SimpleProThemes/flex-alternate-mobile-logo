...
load_plugin_textdomain( 'faml', false, basename( dirname( __FILE__ ) ) . '/languages/' );
...
function flex_alternate_mobile_logo( $title, $inside, $wrap ) {
  ...
  if( ! empty( $mobile_logo_url ) ) {
    $mobile_logo = '<img src="' . $mobile_logo_url. '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '" width="360" height="150" id="mobile-logo" />';
